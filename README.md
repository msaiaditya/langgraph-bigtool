...
The `createAgent` function accepts an optional `workflowOptions` parameter that allows you to configure how the workflow behaves:

### Error Handling

Control how tool execution errors are handled:

```typescript
// Default behavior - errors are thrown (handleToolErrors: false)
const agent = await createAgent({
  llm,
  tools: toolRegistry,
});

// With error handling enabled - errors are caught and returned as messages
const agentWithErrorHandling = await createAgent(
  {
    llm,
    tools: toolRegistry,
  },
...
**When `handleToolErrors: true`:**

- Tool execution errors are caught and returned as error messages
- The workflow continues even if individual tools fail
- Error messages are included in the conversation history
- Useful for production environments where you want graceful degradation

**When `handleToolErrors: false` (default):**

- Tool execution errors are thrown and may interrupt the workflow
- Useful for development and debugging where you want to see errors immediately
- More predictable behavior for testing scenarios

**Note:** Unlike the Python LangGraph SDK which defaults `handle_tool_errors` to `True`, our TypeScript implementation defaults `handleToolErrors` to `False` for better error visibility during development.
...
const toolRegistry = { risky_tool: riskyTool };
...
// This will continue even if the tool fails
const result = await agent.invoke({
  messages: [{ role: "user", content: "Use risky_tool with input 'error'" }],
});
// The result will include an error message instead of throwing
```
...
### `createAgent(input, workflowOptions?)`
...
- `workflowOptions?`: Optional workflow configuration
  - `handleToolErrors?`: Whether to handle tool execution errors gracefully (default: false)
    - When `true`: Tool errors are caught and returned as error messages instead of throwing
    - When `false`: Tool errors are thrown and may interrupt the workflow
...
- The `workflowOptions.handleToolErrors` setting controls how tool execution errors are handled. When enabled, errors are caught and returned as error messages, allowing the workflow to continue. When disabled, errors are thrown and may interrupt the workflow.
...
const defaultTools = {
  ...
  getCurrentTime: tool(async () => new Date().toISOString(), {
    name: "getCurrentTime",
